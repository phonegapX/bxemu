...
class TestStrategy(StrategyTemplate):
    ...
    def onTick(self, tick):
        """收到行情TICK推送"""
        try:
            self.pm.placeOrder(DIRECTION_BUY, 5562.0, 100)
            #
            self.pm.placeOrder(DIRECTION_SELL, 6472.5, 270)
            self.pm.placeOrder(DIRECTION_BUY, 5361.5, 150)
...
class BacktestBitMEXMarket(object):
    ...
    def run(self):  
        ...
        self.account.deposit(112784815) #给账户充值
        self.account.adjustLeverage(3)  #

        tupleQuote1 = (5484.61, 5562.00, datetime.now())
